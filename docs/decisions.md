...
**D23. Reference examples are written by the author.** Olympiad-style problems with trap annotations in `data/examples/tasks.json` (the count is set in T11 from research/12); style and difficulty levels follow Soviet problem books (D36). A task may reuse the mathematical idea and answer of a problem from any source — book, textbook, olympiad archive, open set — recorded in its `source` field; the English wording, plot, 5 options and traps are always ours. Problem ideas are not protected by copyright, their wording is: a freely available book is not necessarily in the public domain, and open contest sets forbid commercial use or AI training (SMART-840 forbids both, MathArena's sets are CC BY-NC-SA; research/02). — *Rejected:* copying problem texts, even translated, from public-domain or permissively licensed sources only (few such sources, a rights check for each, and the options and traps would have to be ours anyway). — SPEC 4.3.

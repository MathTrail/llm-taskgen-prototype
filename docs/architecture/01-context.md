...
>
> **Устарело после перехода на MCP (D42).** Схема ниже описывает прежний конвейер, где наш код вызывал Claude API за четырёх агентов. Теперь текст задач пишет модель AI-клиента, а прототип — MCP-сервер (SPEC 5, 5.8). Схему перерисую в T19.

...
Таблица «Все шаги» и таблица коридора — эталон для `rating.py`. Допуск сравнения — 10⁻⁴. Параметры: K₀ = 0,4, a = 0,05, угадывание 0,2, коридор [0,70; 0,85], β = сложность − 3 (1 → −2, 2 → −1, 3 → 0, 4 → +1, 5 → +2).
...
4. **Стартовая история не двигает β задач.** У записей стартовой истории нет задачи в банке (`task_id` пустой, см. [03](03-data-model.md)). При пересчёте в `seed.py` β берётся по сложности и нигде не сохраняется — обновляются только θ и δ. Стоит записать явно.

...
3. **`requests.source` при создании запроса.** Запрос создаётся сразу после ТЗ, чтобы к нему привязывались попытки. Но итог (`bank`, `generated`, `failed`) в этот момент ещё неизвестен, а `source` — `NOT NULL`. **Сделано в T12:** `db.create_request` пишет `failed`, `db.close_request` ставит настоящий источник. Если `main.py` закоммитит запрос до генерации, запуск, упавший на полпути, останется в журнале как `failed`. Альтернатива — разрешить NULL в `source` («запрос не закрыт»); тогда отчёт (T29) должен считать незакрытые запросы отдельно. Решение за мной. **Уточнено в T21 (MCP):** открытым считается запрос с `source = 'failed'`, без задачи и с `attempt_count` меньше `max_attempts` — только такой принимает `submit_task`. Запрос, который модель бросила, так и остаётся `failed` с `attempt_count = 0`; отчёт T24 может отделить брошенные запросы от провалившихся по этому счётчику.

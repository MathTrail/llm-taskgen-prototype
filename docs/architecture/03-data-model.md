...
1. **`task_id` в стартовой истории.** В примере профиля (SPEC 4.1) у записей истории есть `"task_id": "t-0012"`, а в SPEC 7 у стартовой истории `task_id` пустой. Если записать `t-0012`, сработает внешний ключ: такой задачи в `tasks` нет. Предложение: `seed.py` отбрасывает `task_id` стартовой истории, а из примера в SPEC 4.1 поле убрать.
